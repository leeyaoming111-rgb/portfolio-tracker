      ...
      <div>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 6 }}>
            <div style={{ flex: 1 }}>
              <label style={{ fontSize: 10, color: "#484f58", display: "block", marginBottom: 2 }}>Fair Value ({isUSStock ? "USD" : "MYR"})</label>
...
export default function App() {
  ...
  const [showAllMYR, setShowAllMYR] = useState(true);
  ...
  }, [fetchPortfolio, fetchActions, fetchReturns, fetchDeposits, fetchConviction]);
  ...
  return (
    <div style={{
      ...
      <main style={{ padding: "20px 24px" }}>
        ...
        {activeTab === "Dashboard" && (
          <div className="fade-in">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 16, marginBottom: 20 }}>
              {[
                { label: "Total NAV", value: fmtRm(portfolio.total_nav), sub: null, color: "#e6edf3" },
                { label: "Equity NAV", value: fmtRm(portfolio.equity_nav), sub: portfolio.total_nav > 0 ? `${fmt(portfolio.equity_nav / portfolio.total_nav * 100, 1)}% of NAV` : null, color: "#58a6ff" },
                { label: "Deployable Cash", value: fmtRm(portfolio.cash_plus), sub: portfolio.total_nav > 0 ? `${fmt(portfolio.cash_plus / portfolio.total_nav * 100, 1)}% of NAV` : null, color: "#d29922" },
                { label: "Today's P&L", value: `${(portfolio.today_pl_total || 0) >= 0 ? "+" : ""}${fmtRm(portfolio.today_pl_total || 0)}`, sub: null, color: (portfolio.today_pl_total || 0) >= 0 ? "#3fb950" : "#f85149" },
            ...
            <div className="card">
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                ...
                <div style={{
                  ...
                  <button onClick={() => setShowAllMYR(true)} style={{
                    padding: "6px 16px", fontSize: 12, fontFamily: "inherit", fontWeight: 600,
                    border: "none", cursor: "pointer",
                    background: showAllMYR ? "#d29922" : "#161b22",
                    color: showAllMYR ? "#0d1117" : "#8b949e",
                  }}>All RM</button>
                  <button onClick={() => setShowAllMYR(false)} style={{
                    padding: "6px 16px", fontSize: 12, fontFamily: "inherit", fontWeight: 600,
                    border: "none", cursor: "pointer", borderLeft: "1px solid #30363d",
                    background: !showAllMYR ? "#bc8cff" : "#161b22",
                    color: !showAllMYR ? "#0d1117" : "#8b949e",
                ...
                <table>
                  ...
                  <tbody>
                    {[...portfolio.positions].sort((a, b) => {
                      ...
                      const sym = us ? "US$" : "RM";
                      const valDisplay = showAllMYR ? (p.val_myr || p.val) : p.val;
                      const plDisplay = showAllMYR ? (p.pl_val_myr || p.pl_val) : p.pl_val;
                      const valPrefix = showAllMYR ? "RM" : sym;
                      return (
                      <tr key={i}>
                        ...
                        <td className="mono" style={{ textAlign: "right" }}>
                          ...
                          {showAllMYR && us && <div style={{ fontSize: 10, color: "#bc8cff" }}>US$ {fmt(p.val, 0)}</div>}
                        </td>
                        ...
                        <td className="mono" style={{
                          textAlign: "right", color: p.pl_val >= 0 ? "#3fb950" : "#f85149",
                        }}>
                          <div>{valPrefix} {fmt(Math.abs(plDisplay), plDisplay >= 1000 || plDisplay <= -1000 ? 0 : 2)}</div>
                          {showAllMYR && us && <div style={{ fontSize: 10, color: p.pl_val >= 0 ? "rgba(63,185,80,0.5)" : "rgba(248,81,73,0.5)" }}>US$ {fmt(Math.abs(p.pl_val), 0)}</div>}
                        </td>
                        <td className="mono" style={{
        ...
        {/* ════ RETURNS TAB ════ */}
        {activeTab === "Returns" && (() => {
          // ── Compute returns data — all in MYR ──
          const fx = portfolio.fx_rate || 3.93;
          const toMyr = (p, val) => p.code.startsWith("US.") ? val * fx : val;
          const totalPL = portfolio.positions.reduce((s, p) => s + toMyr(p, p.pl_val || 0), 0);
          const totalUnrealized = portfolio.positions.reduce((s, p) => s + toMyr(p, p.unrealized_pl || 0), 0);
          const totalRealized = portfolio.positions.reduce((s, p) => s + toMyr(p, p.realized_pl || 0), 0);
          const totalCost = portfolio.positions.reduce((s, p) => s + (p.cost_total_myr || p.cost_price * p.qty * (p.code.startsWith("US.") ? fx : 1)), 0);
          const totalReturnPct = totalCost > 0 ? (totalPL / totalCost * 100) : 0;
          const todayPL = portfolio.today_pl_total || 0;

          // Attribution: each stock's contribution to total P&L (in MYR)
          const attribution = portfolio.positions
            .map(p => {
              const plMyr = toMyr(p, p.pl_val || 0);
              return {
                name: p.stock_name,
                code: p.code,
                pl_val: plMyr,
                unrealized: toMyr(p, p.unrealized_pl || 0),
                realized: toMyr(p, p.realized_pl || 0),
                today_pl: toMyr(p, p.today_pl || 0),
                pl_ratio: p.pl_ratio,
                weight_pct: p.weight_pct,
                contribution: totalCost > 0 ? (plMyr / totalCost * 100) : 0,
                market: p.code.startsWith("US.") ? "US" : "MY",
              };
            })
            .sort((a, b) => b.pl_val - a.pl_val);

          // MY vs US breakdown (in MYR)
          const myStocks = attribution.filter(a => a.market === "MY");
          const usStocks = attribution.filter(a => a.market === "US");
          const myPL = myStocks.reduce((s, a) => s + a.pl_val, 0);
          const usPL = usStocks.reduce((s, a) => s + a.pl_val, 0);
          const myValue = portfolio.positions.filter(p => !p.code.startsWith("US.")).reduce((s, p) => s + (p.val_myr || p.val), 0);
          const usValue = portfolio.positions.filter(p => p.code.startsWith("US.")).reduce((s, p) => s + (p.val_myr || p.val), 0);
          ...
          // Y-axis domain for performance chart
          const allChartVals = chartSeries.flatMap(d => {
            const vals = [d.portfolio];
            if (showBenchmark && d.spx) vals.push(d.spx);
            return vals;
          }).filter(Boolean);
          const yMin = allChartVals.length ? Math.floor(Math.min(...allChartVals) - (Math.max(...allChartVals) - Math.min(...allChartVals)) * 0.1 || 2) : 95;
          const yMax = allChartVals.length ? Math.ceil(Math.max(...allChartVals) + (Math.max(...allChartVals) - Math.min(...allChartVals)) * 0.1 || 2) : 105;
            ...
            <div className="fade-in">
              ...
              <div className="card" style={{ marginBottom: 20 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                  <div className="card-header" style={{ marginBottom: 0 }}>Performance — Time-Weighted Return</div>
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
                  {["1D", "1W", "1M", "3M", "YTD", "1Y"].map(key => {
                    ...
                    return (
                      <div key={key} style={{
                        background: "#0d1117", border: `1px solid ${chartPeriod === key || (key === "YTD" && chartPeriod === "YTD") ? "#58a6ff" : "#21262d"}`,
                  ...
                  {returnsLoading ? (
                    ...
                    <ResponsiveContainer width="100%" height={280}>
                      <ComposedChart data={chartSeries.map(d => ({ ...d, dateLabel: formatDateLabel(d.date) }))}>
                        ...
                        <YAxis domain={[yMin, yMax]} tick={{ fill: "#8b949e", fontSize: 11 }} axisLine={false}
                          tickLine={false} tickFormatter={v => v.toFixed(0)} width={40} />
                        <Tooltip
                          contentStyle={{ background: "#161b22", border: "1px solid #30363d", borderRadius: 8, fontSize: 12 }}
                          labelStyle={{ color: "#8b949e" }}
                          formatter={(value, name) => {
                            if (name === "portfolio") return [`${fmt(value)} (${value >= 100 ? "+" : ""}${fmt(value - 100)}%)`, "Portfolio"];
                            if (name === "spx") return [`${fmt(value)} (${value >= 100 ? "+" : ""}${fmt(value - 100)}%)`, "S&P 500"];
                            return [value, name];
                          }}
                        />
                        <ReferenceLine y={100} stroke="#484f58" strokeDasharray="4 4" strokeOpacity={0.5} />
                        <Area type="monotone" dataKey="portfolio" stroke="#58a6ff" fill="url(#gradPortfolio)"
                          strokeWidth={2} dot={false} activeDot={{ r: 4, stroke: "#58a6ff" }} name="portfolio" />
                        {showBenchmark && (
                          <Line type="monotone" dataKey="spx" stroke="#d29922" strokeWidth={1.5}
                            dot={false} strokeDasharray="5 3" activeDot={{ r: 3, stroke: "#d29922" }} name="spx" />
                        )}
                      </ComposedChart>
                    </ResponsiveContainer>
                  )}
                ...
                {returnData && (
                  <div style={{ display: "flex", gap: 12, marginTop: 12, flexWrap: "wrap" }}>
                    {[
                      { label: "Total Deposited", value: fmtRm(returnData.total_deposits) },
                ...
                {/* Legend */}
                <div style={{ display: "flex", gap: 16, marginTop: 12, fontSize: 11, color: "#484f58" }}>
                  <span><span style={{ color: "#58a6ff" }}>━━</span> Portfolio (TWR, deposit-adjusted)</span>
                  {showBenchmark && <span><span style={{ color: "#d29922" }}>╌╌</span> S&P 500 (SPY)</span>}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 16, marginBottom: 24 }}>
                ...
                <div className="card" style={{ textAlign: "center", padding: 24 }}>
                  ...
                  }}>{totalUnrealized >= 0 ? "+" : ""}{fmtRm(totalUnrealized)}</div>
                ...
                <div className="card" style={{ textAlign: "center", padding: 24 }}>
                  ...
                  }}>{totalRealized >= 0 ? "+" : ""}{fmtRm(totalRealized)}</div>
                    ...
                    {attribution.map((a, i) => (
                      <div key={i} style={{
                        ...
                        <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
                          ...
                          }}>{a.pl_val >= 0 ? "+" : ""}{fmtRm(a.pl_val)}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="card">
                  ...
                  <div style={{ marginBottom: 20 }}>
                    ...
                    <div style={{
                      ...
                      {myValue > 0 && (
                        <div style={{
                          width: `${myValue / (myValue + usValue) * 100}%`, height: "100%",
                          background: "linear-gradient(90deg, #58a6ff, #388bfd)",
                          display: "flex", alignItems: "center", justifyContent: "center",
                          fontSize: 11, fontWeight: 600, color: "#fff",
                        }}>MY {fmt(myValue / (myValue + usValue) * 100, 0)}%</div>
                      )}
                      {usValue > 0 && (
                        <div style={{
                          width: `${usValue / (myValue + usValue) * 100}%`, height: "100%",
                          background: "linear-gradient(90deg, #bc8cff, #8b5cf6)",
                          display: "flex", alignItems: "center", justifyContent: "center",
                          fontSize: 11, fontWeight: 600, color: "#fff",
                        }}>US {fmt(usValue / (myValue + usValue) * 100, 0)}%</div>
                      )}
                    </div>
                  </div>
                  ...
                  <div style={{
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                      <span style={{ fontSize: 13, fontWeight: 600 }}>🇲🇾 Bursa Malaysia</span>
                      <span className="mono" style={{ fontSize: 13, color: "#58a6ff" }}>{fmtRm(myValue)}</span>
                    </div>
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                      <span style={{ fontSize: 12, color: "#8b949e" }}>P&L</span>
                      <span className="mono" style={{
                        fontSize: 13, fontWeight: 600,
                        color: myPL >= 0 ? "#3fb950" : "#f85149",
                      }}>{myPL >= 0 ? "+" : ""}{fmtRm(myPL)}</span>
                    </div>
                  ...
                  <div style={{
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                      <span style={{ fontSize: 13, fontWeight: 600 }}>🇺🇸 US Market</span>
                      <span className="mono" style={{ fontSize: 13, color: "#bc8cff" }}>{fmtRm(usValue)}</span>
                    </div>
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                      <span style={{ fontSize: 12, color: "#8b949e" }}>P&L</span>
                      <span className="mono" style={{
                        fontSize: 13, fontWeight: 600,
                        color: usPL >= 0 ? "#3fb950" : "#f85149",
                      }}>{usPL >= 0 ? "+" : ""}{fmtRm(usPL)}</span>
                    </div>
                    <div style={{ marginTop: 6 }}>
                      {usStocks.map((s, i) => (
                        <span key={i} style={{
                          fontSize: 10, padding: "2px 8px", borderRadius: 10, marginRight: 4,
                          background: "rgba(188,140,255,0.1)", color: "#bc8cff",
                        }}>{s.name}</span>
                      ))}
                    </div>
                  </div>
                  ...
                  <div style={{
                    ...
                    📊 See the Performance chart above for S&P 500 benchmark comparison with deposit-adjusted TWR.
                  </div>
                </div>
              </div>
            </div>
                  ...
                  <div key={p.code} style={{
                    ...
                    <div onClick={() => setExpandedConviction(isExpanded ? null : p.code)}
                      ...
                      {fairValue > 0 && (
                        ...
                        }}>FV: {isUS(p.code) ? "$" : "RM"}{fmt(fairValue)} ({marginOfSafety > 0 ? "+" : ""}{fmt(marginOfSafety, 0)}%)</span>
                      )}
                ...
                <tbody>
                  {actions.actions.map((a, i) => (
                    <tr key={i}>
                      ...
                      <td className="mono" style={{ textAlign: "right" }}>{fmtRm(a.value)}</td>
                      <td className="mono" style={{ textAlign: "right", color: "#d29922" }}>
                        {fmtRm(a.running_cash)}
                      </td>
                    </tr>
                  ))}
                </tbody>
                  ...
                  <tbody>
                    {trades.map((t, i) => (
                      <tr key={i}>
                        ...
                        <td className="mono" style={{ textAlign: "right" }}>{fmtRm(t.price)}</td>
                    ...
                    <tbody>
                      {snapshots.map((s, i) => (
                        <tr key={i}>
                          ...
                          <td className="mono" style={{ textAlign: "right" }}>{fmtRm(s.total_nav)}</td>
                          <td className="mono" style={{ textAlign: "right" }}>{fmtRm(s.equity_nav)}</td>
                          <td className="mono" style={{ textAlign: "right", color: "#d29922" }}>{fmtRm(s.cash_plus)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
      ...
      <footer style={{
        ...
        <span>Backed by IBKR Gateway · {isLive ? "Live" : "Demo"} mode · USD/MYR {portfolio.fx_rate || "3.93"}</span>
